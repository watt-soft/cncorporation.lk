...
const ProductCard = ({ title, image, category, description, features, index = 0 }: ProductCardProps) => {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-primary/40 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center z-10">
          <motion.div
            ...
            whileHover={{ scale: 1 }}
            className="flex items-center gap-2 text-accent font-semibold text-sm border border-accent/50 px-5 py-2.5 rounded-full group-hover:opacity-100 opacity-0 transition-opacity duration-500 delay-100"
          >
            <Eye size={16} />
            View Details
          </motion.div>
      ...
      <div className="p-6 flex-grow flex flex-col">
        ...
        <motion.button
          whileHover={{ x: 5, color: '#F4C51B' }}
          transition={{ type: 'spring', stiffness: 300 }}
          className="flex items-center text-accent font-semibold text-sm gap-2 mt-auto w-fit uppercase tracking-wider"
        >
          Request Quote
          <motion.span
            animate={{ x: [0, 5, 0] }}
